...
export const Form: FC<FormProps> = (props) => {
  ...
  return (
    <BootstrapForm className={props.className} onSubmit={formik.handleSubmit}>
      {props.error && !props.submitting && <Alert variant='danger'>{props.error}</Alert>}
      {Object.keys(props.initialValues).map((field: string) => (
        <BootstrapForm.Group key={field + 'Group'} className='mb-2'>
          ...
          <BootstrapForm.Control
            ...
            className={
              formik.touched[field] ? (formik.errors[field] ? 'is-invalid' : 'is-valid') : ''
            }
      ...
      <BootstrapForm.Group className='d-flex justify-content-end'>
        <Button disabled={!formik.isValid || props.submitting} type='submit'>
          Sign in
          {props.submitting && (
            <Spinner
              as='span'
