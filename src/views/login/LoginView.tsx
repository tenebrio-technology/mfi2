...
export const LoginView: React.FC = () => {
  ...
  const authenticationFailed = useSelector(
    (state: RootState) => state.auth.authenticationFailed,
  );
  const authenticating = useSelector(
    (state: RootState) => state.auth.authenticating,
  );
    ...
    <>
      ...
      <Container className="fluid vh-100 ">
        <Row className="h-100 fluid w-100">
          ...
          <Col
            xs={3}
            className="sign-in-col h-100 align-items-center justify-content-center p-0">
            <h3 className="w-100">Sign in</h3>
            <Form className="sign-in-form w-100 p-3" 
